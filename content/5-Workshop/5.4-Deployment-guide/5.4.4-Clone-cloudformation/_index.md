---
title: "Clone the CloudFormation repository"
date: 2026-07-04
weight: 4
chapter: false
pre: " <b> 5.4.4. </b> "
---

Return to the release folder and clone the CloudFormation templates:

```powershell
cd D:\HaShop-release
git clone https://github.com/Leviethaidang/cloudformation.git
```

The repository contains the templates used in the next step:

- `00-cf-hashop-global-waf.yaml`
- `01-cf-hashop-foundation.yaml`
- `02-cf-hashop-ecs-services.yaml`
- `03-cf-hashop-db-init.yaml`

![Clone CloudFormation repository](/fcj-workshop-hungdung/images/5-Workshop/hashop-deployment/image6.png)
